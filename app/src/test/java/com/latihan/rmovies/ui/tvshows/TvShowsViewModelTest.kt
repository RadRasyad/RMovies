...
class TvShowsViewModelTest : TestCase() {
    ...
    fun testGetShows() {
        ...
        `when`(dataRepository.getTvShows()).thenReturn(shows)
    ...
    fun testGetDetailShow() {
        ...
        assertEquals(shows.value!!.id, viewModel?.getDetailShow(shows.value!!.id.toString())?.value?.id)
        assertEquals(shows.value!!.name, viewModel?.getDetailShow(shows.value!!.id.toString())?.value?.name)
        assertEquals(shows.value!!.overview, viewModel?.getDetailShow(shows.value!!.id.toString())?.value?.overview)
        assertEquals(shows.value!!.firstAirDate, viewModel?.getDetailShow(shows.value!!.id.toString())?.value?.firstAirDate)
        assertEquals(shows.value!!.posterPath, viewModel?.getDetailShow(shows.value!!.id.toString())?.value?.posterPath)
        assertEquals(shows.value!!.backdropPath, viewModel?.getDetailShow(shows.value!!.id.toString())?.value?.backdropPath)
    }
}